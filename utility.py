...
def save_image(x,count=0,file_prefix='gen',path='image/'):
    for t in range(T):
        img = xrecons_grid(x[t],B,A)
        plt.matshow(img, cmap=plt.cm.gray)
        imgname = path+file_prefix+'_count_%d_%s_%d.png' % (count,'test', t)  # you can merge using imagemagick, i.e. convert -delay 10 -loop 0 *.png mnist.gif
        plt.savefig(imgname)
        print(imgname)

def save_image_single(x,count=0,file_prefix='gen',path='image/'):
    img = xrecons_grid(x,B,A)
    plt.matshow(img, cmap=plt.cm.gray)
    imgname = path+file_prefix+'_count_%d_%s.png' % (count,'test')  # you can merge using imagemagick, i.e. convert -delay 10 -loop 0 *.png mnist.gif
    plt.savefig(imgname)
    print(imgname)

def save_image_multi(x,count=0):
    torch_save_image(image_gen.view(test_gen_size, 1, 50, 50),
                   'results_rot/sample_' + str(epoch) + '_' + str(i) + '.png')
