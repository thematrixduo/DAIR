...
import torch.utils
from air_model_dis_deep_oc import DrawModel
...
data_path = '/local/sdb/wd263/my_dsprites/'
dataset_npz = 'dsprites_data_och.npz'
model_save_path = 'save/weights_oc_170000.tar'
...
train_loader, X, true_counts, labels, labels_mask = load_data_label(data_path,batch_size,dataset_npz)
print('open Visdom')
...
def train():
    ...
    for epoch in range(epoch_num):
        for batch_idx, (data,_) in enumerate(train_loader):
            ...
            loss = model.loss(data,tau, z_pres_prior_p=partial(z_pres_prior_p, count))
            ...
            if count % 100 == 0:
                ...
                if count % 1000 == 0:
                    ...
                    if count % 10000 == 0:
                        ...
                        if count>30000:
                            torch.save(model.state_dict(),'save/weights_oc_%d.tar'%(count))
                    model.train()

                avg_loss = 0
    #torch.save(model.state_dict(), 'save/weights_oc_final.tar')
    #model.eval()
