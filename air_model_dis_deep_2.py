...
class DrawModel(nn.Module):
    ...
    def forward(self,x,tau):
        ...
        for t in range(self.T):
            ...
            x_dec = F.sigmoid(self.conv_dec(dec_fc.view(-1,64,4,4))-2)
            ...
            self.cs[t] = c_prev + out_canvas * z_gumbel.view(-1,1,1,1)
            #print(self.cs[t])
    ...
    def loss(self,x,tau, z_pres_prior_p,eps=1e-8):
        ...
        x_recons = self.cs[-1]
        loglik = self.compute_loglik(x,x_recons)
    ...
    def generate_single(self,cats,z_where,batch_size=64):
        self.batch_size = batch_size
        ...
        for t in range(self.T-1):
            ...
            #print(out_canvas.size())
            # h_dec = self.decoder_gru(z, h_dec_prev)
            #self.cs[t] = c_prev + out_canvas 
            self.cs[t] = out_canvas
